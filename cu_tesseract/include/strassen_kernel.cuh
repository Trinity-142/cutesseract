...
__host__ void _strassen_rec(const T* A, size_t lda,
    ...
    dim3 block_dim(16, 16);
    dim3 grid_dim((H + block_dim.x - 1) / block_dim.x, (H + block_dim.y - 1) / block_dim.y);

    if (depth < STRASSEN_PARALLEL_LEVELS) {
        cudaStream_t s[7];
        for (int i = 0; i < 7; i++) s[i] = stream_pool[stream_offset++];
        ...
        _strassen_rec<T>(S1_P1, H, S2_P1, H, P1, H, H, next_ws + 0 * child_ws_size, depth + 1, stream_pool, stream_offset, s[0]);
        ...
        _strassen_rec<T>(S1_P7, H, S2_P7, H, P7, H, H, next_ws + 6 * child_ws_size, depth + 1, stream_pool, stream_offset, s[6]);

        for (int i = 0; i < 7; i++) CUDA_CHECK(cudaStreamSynchronize(s[i]));
    } else {
        add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(A11, lda, A22, lda, S1_P1, H, H);
        add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(B11, ldb, B22, ldb, S2_P1, H, H);
        _strassen_rec<T>(S1_P1, H, S2_P1, H, P1, H, H, next_ws, depth + 1, stream_pool, stream_offset, current_stream);

        add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(A21, lda, A22, lda, S1_P2, H, H);
        _strassen_rec<T>(S1_P2, H, B11, ldb, P2, H, H, next_ws, depth + 1, stream_pool, stream_offset, current_stream);

        sub_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(B12, ldb, B22, ldb, S2_P3, H, H);
        _strassen_rec<T>(A11, lda, S2_P3, H, P3, H, H, next_ws, depth + 1, stream_pool, stream_offset, current_stream);

        sub_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(B21, ldb, B11, ldb, S2_P4, H, H);
        _strassen_rec<T>(A22, lda, S2_P4, H, P4, H, H, next_ws, depth + 1, stream_pool, stream_offset, current_stream);

        add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(A11, lda, A12, lda, S1_P5, H, H);
        _strassen_rec<T>(S1_P5, H, B22, ldb, P5, H, H, next_ws, depth + 1, stream_pool, stream_offset, current_stream);

        sub_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(A21, lda, A11, lda, S1_P6, H, H);
        add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(B11, ldb, B12, ldb, S2_P6, H, H);
        _strassen_rec<T>(S1_P6, H, S2_P6, H, P6, H, H, next_ws, depth + 1, stream_pool, stream_offset, current_stream);

        sub_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(A12, lda, A22, lda, S1_P7, H, H);
        add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(B21, ldb, B22, ldb, S2_P7, H, H);
        _strassen_rec<T>(S1_P7, H, S2_P7, H, P7, H, H, next_ws, depth + 1, stream_pool, stream_offset, current_stream);
    }

    add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(P1, H, P4, H, T1, H, H);
    sub_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(T1, H, P5, H, T2, H, H);
    add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(T2, H, P7, H, C11, ldc, H);
    add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(P3, H, P5, H, C12, ldc, H);
    add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(P2, H, P4, H, C21, ldc, H);
    sub_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(P1, H, P2, H, T1, H, H);
    add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(T1, H, P3, H, T2, H, H);
    add_square_kernel<T><<<grid_dim, block_dim, 0, current_stream>>>(T2, H, P6, H, C22, ldc, H);
}
...
__host__ void _gemm_strassen(Matrix<T> &A, Matrix<T> &B, Matrix<T> &C) {
    ...
    _strassen_rec<T>(Ap, S, Bp, S, Cp, S, S, workspace, 0, stream_pool, stream_offset, 0);
    CUDA_CHECK(cudaDeviceSynchronize());

    copy_rect_kernel<<<dim3((M+15)/16, (N+15)/16), block>>>(Cp, S, C.item(), M, N, M);
    CUDA_CHECK(cudaDeviceSynchronize());

    for (size_t i = 0; i < num_streams; i++) CUDA_CHECK(cudaStreamDestroy(stream_pool[i]));
    delete[] stream_pool;
